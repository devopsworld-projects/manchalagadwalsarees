...
export function Footer() {
  ...
  const renderFooterLinks = (isMobile = false) => {
    // If we have dynamic footer menu groups, render them
    if (footerMenuItems.length > 0) {
      return footerMenuItems.map(group => (
        <div key={group.id}>
          <h4 className={`font-display ${isMobile ? 'text-base' : 'text-lg'} font-semibold text-primary-foreground ${isMobile ? 'mb-3' : 'mb-4'}`}>
            {group.label}
          </h4>
          <ul className="space-y-2 font-body text-sm">
            {(group.children || []).map(child => (
              <li key={child.id}>
                <Link to={getItemUrl(child)} className="hover:text-gold transition-colors">
                  {child.label}
                </Link>
              </li>
            ))}
    ...
    return (
      <>
        <div>
          <h4 className={`font-display ${isMobile ? 'text-base' : 'text-lg'} font-semibold text-primary-foreground ${isMobile ? 'mb-3' : 'mb-4'}`}>Quick Links</h4>
          <ul className="space-y-2 font-body text-sm">
        ...
        <div>
          <h4 className={`font-display ${isMobile ? 'text-base' : 'text-lg'} font-semibold text-primary-foreground ${isMobile ? 'mb-3' : 'mb-4'}`}>Categories</h4>
          <ul className="space-y-2 font-body text-sm">
  ...
  const contactSection = (isMobile = false) => (
    <div>
      <h4 className={`font-display ${isMobile ? 'text-base' : 'text-lg'} font-semibold text-primary-foreground ${isMobile ? 'mb-3' : 'mb-4'}`}>Contact Us</h4>
      <ul className={`space-y-${isMobile ? '2.5' : '3'} font-body text-sm`}>
  ...
  return (
    <footer className="hidden md:block bg-foreground text-primary-foreground/80">
      ...
      <div className="md:hidden">
        ...
        {expanded && (
          <div className="px-6 pb-6 space-y-6 animate-in slide-in-from-bottom-2 duration-300">
            <div>
              <p className="font-body text-sm leading-relaxed text-primary-foreground/60">{description}</p>
              {socialIcons}
            </div>
      ...
      <div className="container py-12 md:py-16 hidden md:block">
        <div className={`grid md:grid-cols-${Math.min(footerMenuItems.length + 2, 4)} gap-8`}>
          <div>
