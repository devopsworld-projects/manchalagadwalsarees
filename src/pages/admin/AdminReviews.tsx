...
export default function AdminReviews() {
  ...
  const bulk = useBulkSelect(reviews);
  ...
  const bulkDel = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-reviews'] }); toast.success(`${bulk.count} reviews deleted`); bulk.clear(); },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {bulk.someSelected && (
          <Button variant="destructive" onClick={() => { if (confirm(`Delete ${bulk.count} selected reviews?`)) bulkDel.mutate(Array.from(bulk.selectedIds)); }} disabled={bulkDel.isPending}>
      ...
      {isLoading ? <p className="text-center py-12 text-muted-foreground">Loading...</p> : reviews.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {reviews.map((r: any) => (
            <div key={r.id} className={`border rounded-lg p-4 ${bulk.selectedIds.has(r.id) ? 'bg-primary/5 border-primary/30' : ''}`}>
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-start gap-3">
                  <Checkbox checked={bulk.selectedIds.has(r.id)} onCheckedChange={() => bulk.toggle(r.id)} className="mt-1" />
                  <div>
              ...
              {r.comment && <p className="text-sm text-muted-foreground mb-3 ml-8">{r.comment}</p>}
