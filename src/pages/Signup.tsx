...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (error) { toast.error(error.message); } else { toast.success('Check your email to verify your account'); navigate('/login'); }
  };
      ...
      <main className="relative min-h-[80vh] flex items-center justify-center py-16 px-4">
        ...
        <div className="relative w-full max-w-sm">
          ...
          <div className="relative border border-border bg-card p-8">
            ...
            <button onClick={handleGoogleSignIn} className="w-full flex items-center justify-center gap-3 border border-border py-3 font-body text-sm hover:bg-muted transition-colors">
            ...
            <div className="relative my-6">
              <div className="absolute inset-0 flex items-center"><div className="w-full ornate-line" /></div>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              <div>
                <label className="font-display text-[10px] font-bold tracking-[0.15em] uppercase text-foreground/70 mb-1.5 block">Full Name</label>
                <div className="relative">
                  <User className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                  <Input value={name} onChange={e => setName(e.target.value)} className="pl-10 h-11 font-body border-border" placeholder="Your full name" required />
                </div>
              </div>
              <div>
